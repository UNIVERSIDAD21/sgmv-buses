...
test('P7 conserva trazabilidad tecnica y proyecta disponibilidad segura al despacho', async ({
  ...
  await expect(projection.getByText('Disponible para operación', { exact: true })).toBeVisible()
  await expect(projection.getByText(/Cierre administrativo:.*sin restricciones/i)).toBeVisible()
